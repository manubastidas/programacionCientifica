...
\end{lstlisting}
	...
	\begin{frame}{El límite de Newton-Cotes}
		...
		\vspace{8pt}
		\begin{center}
				\textcolor{red}{\texttt{scipy.integrate.quadrature}}
		\end{center}
	\end{frame}
	...
	% ----------
	\begin{frame}{El volumen de una Hiperesfera (10D)}
		\vspace{4pt}
		En 2D, estimamos el área de un círculo ($x_1^2 + x_2^2 \le 1$). \\ \vspace{4pt}
		¿Qué pasa si queremos calcular el volumen de una \textbf{hiperesfera en 10 dimensiones}?
		
		\vspace{6pt}
		\begin{columns}[T]
			\column{0.5\textwidth}
			\begin{obsbox}{La Condición Analítica}
				Un punto $(x_1, x_2, \dots, x_{10})$ pertenece a la hiperesfera de radio 1 si:
				\[
				\sum_{i=1}^{10} x_i^2 \le 1
				\]
			\end{obsbox}
			
			\column{0.48\textwidth}
			\begin{defbox}{El fracaso de los métodos clásicos}
				Si usamos la Regla de Simpson con una malla muy pobre de \textbf{apenas 10 puntos} por eje:
				\begin{itemize}
					\item En 2D: $10^2 = 100$ puntos.
					\item En 10D: $10^{10} = 10.000.000.000$ evaluaciones.
				\end{itemize}
			\end{defbox}
		\end{columns}
	\end{frame}
