...
### Capa Fourier (15 pts)
- [5] La función que construye la capa Fourier (W1 con filas de senos y cosenos). (15/15... ver nota)
...
(**Comentario**:solo compara el modo $k=1$ no los $K=25$ modos completos)
...
- [8] **Gráfica/análisis del régimen indicado:** contrastar velocidad de convergencia para el régimen que indica el enunciado.
...
(**Comentario**: Ordena los coeficientes por magnitud descendente (`np.argsort(np.abs(X_fft))[::-1]`), correcto pero usa `np.fft.rfft` de forma independiente en vez de referenciar explícitamente la capa Fourier $W_1$ de $K=25$ modos de la red.)
