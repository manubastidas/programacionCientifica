...
> ⚙️ **Enunciado personalizado — ID 3331.** Los parámetros son únicos. 
...
2. Carpeta con los **apellidos** en la raíz del repositorio:
   ...
   ├── enunciado_3331.md   # el enunciado personalizado
   └── datos_3331.npz      # los datos
   ```
...
(**Comentario**:  La comparación con `np.fft` no es clara, no da lo mismo)
...
(**Comentario**: Solo grafica la reconstrucción para el régimen 1, faltan los otros dos regímenes. La entrega original entrenaba con n_iter=10000 y eta=0.1 (no los 3000/0.05 sugeridos) para poder ver convergencia real)
...
(**Comentario**: La curva está semilog-y (no loglog) y con un título erroneo. La medición de iteraciones hasta error<0.1 está triplicada en celdas idénticas y, entrenando solo con los 3000 iters del enunciado, nunca cruza el umbral pero no se hizo el ejercicio de extender las iteraciones). 
...
(**Comentario**: No se reporta explícitamente el error asociado al número de coeficientes del umbral.)
