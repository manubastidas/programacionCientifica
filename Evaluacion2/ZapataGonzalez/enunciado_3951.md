...
> ⚙️ **Enunciado personalizado — ID 3951.** Los parámetros son únicos. 
...
- Entrenar la red con las **dos inicializaciones** (Fourier y aleatoria) sobre señales de los tres regímenes, durante **3000 iteraciones** (learning rate sugerido $\eta=0.02$), y estudiar la evolución de la pérdida (error) en cada caso.
...
2. Carpeta con los **apellidos** en la raíz del repositorio:
   ...
   ├── enunciado_3951.md   # el enunciado personalizado
   └── datos_3951.npz      # los datos
   ```
...
- [1] Evidencia de que, en su inicialización, la capa reproduce la transformada de Fourier de la señal (comparación con np.fft).
- [5] **Reporta la energía media del dataset** ((X**2).mean(), con 3 decimales).

(**Comentario**: La verificación contra `np.fft` es cuantitativa pero solo compara la magnitud del modo k=1, no los K=25 modos completos)

### Entrenamiento y reconstrucción (25 pts) 
- [9] **Gráfica:** la señal original junto a su reconstrucción final, para una señal de cada régimen.
...
(**Comentario**: Sigo sin encontrar el problema porque tu red da tan malos resultados.)
