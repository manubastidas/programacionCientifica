...
> ⚙️ **Enunciado personalizado — ID 1151.** Los parámetros son únicos. 
...
- Entrenar la red con las **dos inicializaciones** (Fourier y aleatoria) sobre señales de los tres regímenes, durante **4000 iteraciones** (learning rate sugerido $\eta=0.02$), y estudiar la evolución de la pérdida (error) en cada caso.
...
2. Carpeta con los **apellidos** en la raíz del repositorio:
   ...
   ├── enunciado_1151.md   # el enunciado personalizado
   └── datos_1151.npz      # los datos
   ```
...
(**Comentario**: Verificación cuantitativa contra np.fft con gráfica)
...
(**Comentario**:  entrena una red separada por régimen. ¿Qué significa la desviación del error?)
...
(**Comentario**: la curva de pérdida usa solo escala log en y. La medición de iteraciones hasta error<0.1 se hace solo con las 3000 iteraciones del enunciado, sin extender el entrenamiento cuando no se cruza el umbral)
