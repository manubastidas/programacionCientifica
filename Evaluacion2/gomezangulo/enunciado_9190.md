...
> ⚙️ **Enunciado personalizado — ID 9190.** Los parámetros son únicos. 
...
2. Carpeta con los **apellidos** en la raíz del repositorio:
   ...
   ├── enunciado_9190.md   # el enunciado personalizado
   └── datos_9190.npz      # los datos
   ```
...
(**Comentario**: No se compara cuantitativamente contra `np.fft`: solo se grafica un stem de los coeficientes `x @ W1`, sin verificación de error.)
OJO: EL CÓDIGO NO TIENE COMENTARIOS. ¿Lo entiendes línea a línea?
...
(**Comentario**: Se entrena una red distinta por régimen y en régimen 0 y 1 falta la reconstrucción/error de la inicialización aleatoria.
No usa los estilos de latex, gráficas muy simples)
...
(**Comentario**: las curvas de pérdida están superpuestas por régimen, pero en escala lineal, no loglog. No hay medición real de iteraciones hasta error relativo < 0.1, y además su umbral por defecto (0.15) es sobre la pérdida, no sobre el error relativo del 0.1 pedido.)
...
(**Comentario**: Para los cálculos usa fft en lugar de la matriz construida en el ejercicio) 
