...
> ⚙️ **Enunciado personalizado — ID 8414.** Los parámetros son únicos. 
...
2. Carpeta con los **apellidos** en la raíz del repositorio:
   ...
   ├── enunciado_8414.md   # el enunciado personalizado
   └── datos_8414.npz      # los datos
   ```
...
- [10] Error de reconstrucción relativo $\|\hat x - x\|/\|x\|$ reportado para cada caso.

(**Comentario**: Usas keys diferentes y esto no sigue las instrucciones, no entiendo cómo logra errores de cero, esto tiene sentido? ¿Qué parte del entrenamiento no está bien?)

### Comparación de inicializaciones (25 pts)
- [8] **Gráfica:** curvas de la pérdida $\mathcal{L}$ vs iteraciones, con las dos inicializaciones superpuestas en la misma figura.
- [2] Medición de **cuántas iteraciones** necesita cada inicialización para que el **error relativo** 0.1.
- [10] **Gráfica/análisis del régimen indicado:** contrastar velocidad de convergencia para el régimen que indica el enunciado.

(**Comentario**: Las gráficas son mejores en loglog)

### Compresión (10 pts)
- [5] **Gráfica:** error de reconstrucción vs número de coeficientes conservados.
- [0] Cuántos coeficientes bastan para lograr umbral de energía del enunciado.

(**Comentario**: Porque usas rfft y no la matriz que acabaste de construir? Solo ordenas las frecuencias pero no las reorganiza y en el régimen alto esto es crucial)